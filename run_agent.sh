...
if test -d alprina_stock_analyst_agent; then
  echo "Removing previous agent build"
  rm -r alprina_stock_analyst_agent
fi
...
# Fetch the agent
autonomy fetch --local --agent victorpolisetty/alprina_stock_analyst_agent
...
cd alprina_stock_analyst_agent
